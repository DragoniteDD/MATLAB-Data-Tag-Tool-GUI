function varargout = PPGa_Label_Tool(varargin)
% PPGA_LABEL_TOOL MATLAB code for PPGa_Label_Tool.fig
%      PPGA_LABEL_TOOL, by itself, creates a new PPGA_LABEL_TOOL or raises the existing
%      singleton*.
%
%      H = PPGA_LABEL_TOOL returns the handle to a new PPGA_LABEL_TOOL or the handle to
%      the existing singleton*.
%
%      PPGA_LABEL_TOOL('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in PPGA_LABEL_TOOL.M with the given input arguments.
%
%      PPGA_LABEL_TOOL('Property','Value',...) creates a new PPGA_LABEL_TOOL or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before PPGa_Label_Tool_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to PPGa_Label_Tool_OpeningFcn via varargin.
%
...
% Edit the above textHistory to modify the response to help PPGa_Label_Tool

% Last Modified by GUIDE v2.5 29-Apr-2018 23:39:58

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @PPGa_Label_Tool_OpeningFcn, ...
                   'gui_OutputFcn',  @PPGa_Label_Tool_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before PPGa_Label_Tool is made visible.
function PPGa_Label_Tool_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to PPGa_Label_Tool (see VARARGIN)

% Choose default command line output for PPGa_Label_Tool
...
% UIWAIT makes PPGa_Label_Tool wait for user response (see UIRESUME)
% uiwait(handles.figure1);


% --- Outputs from this function are returned to the command line.
function varargout = PPGa_Label_Tool_OutputFcn(hObject, eventdata, handles) 
...
if idx ~= int32(idx)
    warndlg('\fontsize{14}Please enter an integer as subject index.', 'Wrong Index', handles.warndlg_opts);
else
    if idx + length(handles.preview_sample) > length(handles.history)
        warndlg('\fontsize{14}Please do not skip your latest subject/study.', 'Warning', handles.warndlg_opts);
